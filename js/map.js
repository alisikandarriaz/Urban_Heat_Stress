...
APP.initMap = function () {
  ...
  Object.entries(APP.WMS_CONFIG).forEach(([id,cfg]) => {
    APP.WMS[id] = L.tileLayer.wms(cfg.url, { layers:cfg.layer, format:'image/png', transparent:true, attribution:'\u00a9 Copernicus', ...cfg.opts });
  });
...
APP._findByNuts = function (col, nuts3) {
  ...
  if (!key) { console.warn('[WFS] No NUTS_ID prop. Available:', Object.keys(sample)); return null; }
...
APP.setCity = async function (key) {
  ...
  // Stats pills on map (LST, UHI, green% — no live temp, that's in header)
...
APP._addReferenceLayers = function (key, c) {
  const nStyle = { color:'#6B7280', weight:2, fill:false, dashArray:'8 5' };
  const sStyle = { color:'#F97316', weight:2, fill:false, dashArray:'10 6' };
  const nf = APP._findByNuts(APP.wfsCache.nuts,      c.nuts3);
  const sf = APP._findByNuts(APP.wfsCache.studyArea, c.nuts3);
  APP.LG.nuts  = nf ? L.geoJSON(nf, {style:nStyle}) : L.rectangle(APP._fb(key), nStyle);
  if (nf) APP.curFeat = nf;
  const fb=APP._fb(key), buf=0.03;
  APP.LG.study = sf ? L.geoJSON(sf, {style:sStyle}) : L.rectangle([[fb[0][0]-buf,fb[0][1]-buf],[fb[1][0]+buf,fb[1][1]+buf]], sStyle);
